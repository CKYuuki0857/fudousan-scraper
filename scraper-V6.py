...
"""
============================================================
 房產戰情室 · 實價登錄抓取器 v6（本機跑 · 自動上傳 GitHub · 手機直接讀）
...
def _gh_conf():
    """
    讀取上傳設定。優先順序：環境變數 > 同資料夾 github_conf.json
    github_conf.json 範例（放在跟這支程式同一個資料夾，切勿上傳到公開倉庫）：
      {
        "owner": "CKYuuki0857",
        "repo":  "fudousan-scraper",
        "branch":"main",
    ...
    p=os.path.join(here,"github_conf.json")
    if os.path.exists(p):
        try:
            with open(p,encoding="utf-8") as f: conf=json.load(f)
        except Exception as e:
            print(f"   ⚠️ github_conf.json 讀取失敗：{e}")
    conf["owner"] =os.environ.get("GH_OWNER") or conf.get("owner","")
    conf["repo"]  =os.environ.get("GH_REPO")  or conf.get("repo","")
    conf["branch"]=os.environ.get("GH_BRANCH") or conf.get("branch","main")
    conf["token"] =os.environ.get("GH_TOKEN")  or conf.get("token","")
    return conf

def upload_github(local_path, remote_path=None, conf=None):
    """把檔案 PUT 到 GitHub（Contents API）。回傳 True/False。"""
    import base64
    conf=conf or _gh_conf()
    if not (conf.get("owner") and conf.get("repo") and conf.get("token")):
        print("\n☁️ （未設定 GitHub 上傳；要用手機同步請建 github_conf.json 或設環境變數 GH_OWNER/GH_REPO/GH_TOKEN）")
        return False
    remote_path = remote_path or os.path.basename(local_path)
    api=f"https://api.github.com/repos/{conf['owner']}/{conf['repo']}/contents/{remote_path}"
    hdr={"Authorization":"Bearer "+conf["token"],
         "Accept":"application/vnd.github+json",
         "X-GitHub-Api-Version":"2022-11-28",
         "User-Agent":"fudousan-scraper"}
    try:
        with open(local_path,"rb") as f: raw=f.read()
        b64=base64.b64encode(raw).decode()
        # 先查現有 sha（有就是更新、沒有就是新增）
        sha=None
        g=requests.get(api, headers=hdr, params={"ref":conf["branch"]}, timeout=20)
        if g.status_code==200:
            sha=g.json().get("sha")
        elif g.status_code not in (404,):
            print(f"   ⚠️ 查詢遠端檔案回應 {g.status_code}：{g.text[:120]}")
        body={"message":f"更新 {remote_path} {datetime.datetime.now():%Y-%m-%d %H:%M}",
              "content":b64, "branch":conf["branch"]}
        if sha: body["sha"]=sha
        r=requests.put(api, headers=hdr, data=json.dumps(body), timeout=30)
        if r.status_code in (200,201):
            print(f"   ✅ 已上傳 {remote_path} → {conf['owner']}/{conf['repo']}@{conf['branch']}")
            return True
        print(f"   ❌ 上傳失敗 HTTP {r.status_code}：{r.text[:200]}")
        if r.status_code==401: print("      → token 無效或過期")
        if r.status_code==403: print("      → token 權限不足（需 Contents: Read and write）")
        if r.status_code==404: print("      → owner/repo/branch 打錯，或 token 沒有這個倉庫的權限")
        return False
    except Exception as e:
        print(f"   ❌ 上傳例外：{e}")
        return False

def upload_results(paths):
    conf=_gh_conf()
    if not (conf.get("owner") and conf.get("repo") and conf.get("token")):
        print("\n☁️ （未設定 GitHub 上傳；跑完的 records.json 仍在本機，可手動上傳）")
        return
    print("\n☁️ 上傳到 GitHub…")
    for p in paths:
        if p and os.path.exists(p): upload_github(p)
    print(f"   🔗 戰情室雲端網址：https://raw.githubusercontent.com/{conf['owner']}/{conf['repo']}/{conf['branch']}/records.json")
...
def output(records, name="records", quiet_clip=False):
    ...
    if quiet_clip:
        print("\n☁️ 雲端模式：records.json 已更新。")
    elif copy_clip(compact):
        print("\n📋 已自動複製到剪貼簿！直接到「房產戰情室 → 行情估價 → 匯入框」貼上即可。")
    else:
        print("\n📋（沒裝 pyperclip，無法自動複製）請整段複製下面這串，貼到「行情估價 → 匯入框」：\n")
        print(compact)

    # 自動上傳 GitHub（有設定才會動作）→ 手機就能用「☁️ 雲端載入」
    if not NO_UPLOAD:
        upload_results([fixed, named])
...
def parse_args(argv):
    ...
    while i < len(argv):
        ...
        if t=='--no-upload':
            globals()['NO_UPLOAD']=True; i+=1; continue
        # 位置參數（相容 v4）
        if not t.startswith('-'):
...
def main():
    ...
    # ── 互動選單（桌機，跟 v4 一樣）
    print("="*60)
    print(f" 房產戰情室 · 抓取器 v6（5168 純淨版 · 自動上傳）   今 民國 {NOW_MINGUO} 年")
    print("="*60)
    print(" 【日常 · 5168 貼了就跑】")
